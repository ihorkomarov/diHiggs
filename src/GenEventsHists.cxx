...
GenEventsHists::GenEventsHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  book<TH1F>("pt_e", "p_{T}^{#e} [GeV/c]", 40, 0, 200);
  book<TH1F>("eta_e", "#eta^{#e}", 40, -2.1, 2.1);
...
void GenEventsHists::fill(const Event & event){
  ...
  // hist("N_ph")->Fill(Nphotons, weight);
  // for (const Photon & thisPh : *event.photons){
  //     hist("pt_ph")->Fill(thisPh.pt(), weight);
  //     hist("eta_ph")->Fill(thisPh.eta(), weight);
  //     //hist("reliso_ph")->Fill(thisPh.relIso(), weight);
  // }
